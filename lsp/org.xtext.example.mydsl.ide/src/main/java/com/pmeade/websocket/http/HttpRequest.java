 ...
 * construction time, it parses the headers of the HTTP Request from the
 * provided InputStream, and places them into an accessible but immutable Map.
 * 
 * @author blinkdog
 */
public class HttpRequest implements Map<String, String> {
	...
	/**
	 * The Request Line of the HTTP Request is stored in the map under this key.
	 */
	 ...
	 * 
	 * @param in InputStream providing the content of the HTTP Request to be parsed
	 */
	...
	/**
	 * Parse the headers of the HTTP Request. The provided InputStream will contain
	 * HTTP Request data. The headers are parsed and placed into the Map. The Map is
	 * then made immutable, for safe querying by clients.
	 * 
	 * @param in InputStream providing the content of the HTTP Request to be parsed
	 */
	private void read(final InputStream in) {
		...
		try {
			...
			// log it, and move on
			LOG.log(java.util.logging.Level.SEVERE, "Unable to read HTTP Request in HttpRequest.read():");
		}
	...
	/**
	 * A Map to store the header fields and values after they are parsed from the
	 * HTTP Request. The parsing function also makes this map unmodifiable, so that
	 * it is safe to allow clients to query it.
	 */
